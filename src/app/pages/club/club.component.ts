...
@Component({
  selector: 'app-club',
  imports: [CommonModule, UiIconComponent, ExternalLinksComponent, I18nPipe, ChipGroupComponent, GameOverviewComponent],
...
export class ClubComponent implements OnDestroy {
  ...
  lastGamesAgainstClub$ = new BehaviorSubject<BasicGame[]>([]);
  ...
  onClubResolved(clubResponse: GetClubByIdResponse): void {
    ...
    if (this.clubResponse.allGames) {
      const enhancedGames = this.clubResponse.allGames.map(game => ({ ...game, opponent: this.clubResponse.club }));

      this.lastGamesAgainstClub$.next(enhancedGames);
    }
